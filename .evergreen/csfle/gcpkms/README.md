...
The instance image family defaults to `debian-12` and may be overridden with `GCPKMS_IMAGEFAMILY`. The image must ship Python 3.9 or newer, which drivers-evergreen-tools needs to install uv on the instance. `debian-12` (the default) and `debian-11` both qualify; `ubuntu-2004` and older do not.
